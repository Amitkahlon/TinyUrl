...
var decodeBase62 = function (chars) {
    return chars.split('').reverse().reduce(function (prev, curr, i) {
        return prev + (base62Encoder.charset.indexOf(curr) * (Math.pow(62, i)));
    }, 0);
};
var isValidChars = function (id) {
    for (var c in id) {
        var isExists = base62Encoder.charset.includes(id);
        if (!isExists) {
            return false;
        }
    }
